...
// ── PHIÊN BẢN ─────────────────────────────────────────────────
String currentVersion = "4.0.2"; // Đã nâng cấp lên 4.0.2
...
#define ALERT_COOLDOWN_MS  (30UL * 1000)
...
void checkAlerts(float tds, float turb) {
  ...
  if (tds > tdsTh) {
    ...
    alTDS = false;
    fbPutJson("/config/notifMuted", "false");
    notifMuted = false;
    snprintf(msg, sizeof(msg),
      "✅ <b>TDS đã về bình thường</b>\n"
      "📊 Hiện tại: <b>%.0f ppm</b> (ngưỡng %.0f ppm)\n"
      "🕐 Lúc: %s\n"
      "🔔 Thông báo đã được bật lại tự động.",
      tds, tdsTh, timeStr());
    tgSend(msg);
  }
...
void setup() {
  ...
  delay(200);
  Serial.println(F("\n╔══════════════════════════════╗"));
  Serial.println(F("║  BỂ CÁ IoT v4.0.2 KHỞI ĐỘNG  ║"));
  Serial.println(F("╚══════════════════════════════╝"));
...
void loop() {
  ...
  lastTDS  = toTDS(rawTDS);
  lastTurb = toTurb(rawTurb);
